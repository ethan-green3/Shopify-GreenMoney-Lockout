...
func TestMoneyEUWebhookHandlerNestedOrderIDFallback(t *testing.T) {
	body := []byte(`{"success":true,"message":"Payment processed successfully","data":{"orderId":"ORD-987","transactionStatus":"PAID"}}`)
	db, state, err := testsql.Open([]testsql.Expectation{
		{
			Kind:          "query",
			QueryContains: "SELECT shop_domain, amount, currency, shopify_order_id FROM money_eu_payments WHERE shopify_order_id = $1",
		...
		{
			Kind:          "query",
			QueryContains: "WHERE money_eu_order_id = $1",
			Args:          []any{"ORD-987"},
			Columns:       []string{"shop_domain", "amount", "currency", "shopify_order_id"},
			Rows:          [][]driver.Value{{"secondary.myshopify.com", float64(49.99), "USD", "123"}},
		},
		{
			Kind:          "exec",
			QueryContains: "UPDATE money_eu_payments SET",
			Args:          []any{"paid", string(body), "secondary.myshopify.com", "123"},
			RowsAffected:  1,
		},
		{
			Kind:          "query",
